  ...
  $.fn.pxuPhotoset = function (options, callback) {
    ...
    $(settings.photoWrap)
      .on("mouseenter", function () {
        $(this).find('.icons').css("visibility", "visible");
      })
      .on("mouseleave", function () {
        $(this).find('.icons').css("visibility", "hidden");
      });
    ...
    $("span.info")
      .on("mouseenter", function () {
        ...
        exifData.css('display', 'block').stop(true, false).animate({
          opacity: 1
        }, 200);
      });
    $("span.info")
      .on("mouseleave", function () {
        var toggle = $(this);
        var exifData = toggle.children('.pxu-data');
        exifData.stop(true, false).animate({
          opacity: 0
        }, 200, function () {
    ...
    return this.each(function () {
      ...
      for (var i = 1; i <= rowCount; i++) {
        ...
        $this.find(settings.photoWrap)
          .slice(
            firstPhoto,
            rowArray[i][2]).addClass(settings.countPrefix + 'count-' + rowArray[i][1]
          ).wrapAll('<div class="row ' + settings.clearfix + '" />');
